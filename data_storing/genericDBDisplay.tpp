...
#include "../TUI_functions/styling_functions.h"
...
Render DB<DT, MaxData>::renderTable(TableStyle ts, char view) {
    ...
    std::string hor_div = outer_ver_div + ts.inner_border_col + std::string(table_width - 2, ts.inner_border_hor_symb) +
                          outer_ver_div;;
    ...
    if (ts.has_header) {
        ...
        for (size_t i = 0; i < columns.size(); ++i) {
            header.append(ts.column_title_style + padString(columns[i].first, column_widths[i] + ts.extra_padding,
                                                            ts.header_padding_symb, ts.centered) + ENDSTY);
...
template<typename DT, unsigned int MaxData>
KeyID DB<DT, MaxData>::display(char view, bool picker, const std::string &title) {
    auto ts = TableStyle();
    ...
    do {
        ...
        if (!title.empty()) {
            std::cout << BLACKSTY
                    BGBRIGHTGREENSTY << padString(title, render.width, ' ', true) << ENDSTY << std::endl;
        }
        ...
        const int user_choice = getIntFromUser(-5, render.pages.size(), ">", true, true);
...
template<typename DT, unsigned int MaxData>
KeyID DB<DT, MaxData>::pickByUser(bool &successful, const std::string &question) {
    if (isEmpty()) {
        successful = false;
        return 0;
    }
